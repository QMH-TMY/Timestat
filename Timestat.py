...
class TimeManage():
	def __init__(self):
		...
		self.inbox_dir  = '/home/shieber/files/gitproject/timestat/inbox.txt'  #暂存邮件的位置
		self.excel_dir  = '/home/shieber/files/gitproject/timestat/'     #excel存放路径 
		self.basename   = "timestat.xlsx"               #excel表基本名,前面会自动要加上年和路径信息
		self.date_str   = 'date'
	...
	def write_to_excel(self):
		...
		for key in keys:
			...
			if key_title not in sheets:
				wb.create_sheet(1,key_title)     #为新的事情添加分表(不常执行, 依邮件的信息而执行)
...
class EmailManage():
	...
	def download_write(self, index, server):
		...
		if self.identifier in value:             #标题含有关键的标识符时执行
			...
			self.timemanage.write_to_excel()     #调用TimeManage的方法,写入excel(核心函数)
